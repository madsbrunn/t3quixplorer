...
//		if ((' '+textareas[z].className+' ').indexOf(' enable-extended-textarea ')>-1) {
		if ((' '+textareas[z].className+' ').indexOf(' enable-extended-textarea ')>-1 || (' '+textareas[z].className+' ').indexOf(' formField3 ')>-1) {
...
function textArea(element,conf) {
	...
	if (Number(this.conf['showWrapButton']) && Number(this.conf['showButtons'])) {
		...
		addEvent(this.wraptoggle, 'click', function(e) { ta.toggleWrap(e); }, false)
	}
	...
	if (!window.opera) {
		...
		if (Number(this.conf['showLinenumButton']) && Number(this.conf['showButtons'])) {
			...
			addEvent(this.linenumtoggle, 'click', function(e) { ta.toggleLineNum(e); }, false);
		}
		...
		if (Number(this.conf['showFindButton']) && Number(this.conf['showButtons'])) {
			...
			addEvent(this.findbutton, 'click', function(e) { var ss = prompt(ta.lang.getLL('findPrompt','Enter string to find'),ta.searchString);if (ss!=null) {ta.searchString = ss;ta.find(e);} else ta.element.focus(); }, false);
		}
		...
		if (Number(this.conf['showJumpButton']) && Number(this.conf['showButtons'])) {
			...
			addEvent(this.jumpbutton, 'click', function(e) { ta.jumpToLine(); }, false);
		}
	}
	else {
		// Opera
	...
	if (Number(this.conf['showFontButtons']) && Number(this.conf['showButtons'])) {
		...
		addEvent(this.fsupbutton, 'click', function(e) { ta.changeFontSize(1); }, false);
		...
		addEvent(this.fsdownbutton, 'click', function(e) { ta.changeFontSize(-1); }, false);
	}
	...
	if (Number(this.conf['showMinMaxButton']) && Number(this.conf['showButtons'])) {
		...
		addEvent(this.minmaxtoggle, 'click', function(e) { ta.minMaximize(e); }, false);
	}
	...
	// Add Tabkey catching
	if (this.catchtab) {
		addEvent(this.element, 'keydown', function(e) { ta.catchTab(e); }, false);
		addEvent(this.element, 'blur', function(e) { ta.blurCallback(e); }, false);
	}
	...
	addEvent(this.grip, 'mousedown', function(e) { ta.beginDrag(e); }, false);
	...
	addEvent(this.element,'focus',function(e) { element.style.color='#000000'; }, false);
	addEvent(this.element,'blur',function(e) { element.style.color='#555555'; }, false);
	...
	if (!window.opera) {
		if (document.all) {
			...
			addEvent(this.element,'keydown', function (e) { if (event.keyCode==13||event.keyCode==8||event.keyCode==46) {window.setTimeout(function() { ta.updateNum(); }, 1); }; },false);
			//addEvent(this.element,'cut', function (e) { window.setTimeout(function() { ta.updateNum();numwrapper.scrollTop = element.scrollTop; }, 1); },false);
			//addEvent(this.element,'paste', function (e) { window.setTimeout(function() { ta.updateNum();numwrapper.scrollTop = element.scrollTop; }, 1); },false);
		}
			...
			addEvent(this.element,'DOMAttrModified', function (e) {ta.mutationEvent(e)}, false);
			...
			addEvent(this.element,'input', function (e) {ta.updateNum();}, false);
			addEvent(this.element,'overflow', function (e) {ta.updateNum();}, false);
		}
	}
...
textArea.prototype.createButton= function(img, altimg, title, alttitle){
		...
		addEvent(button, 'mouseover', function(e) { button.style.border='2px outset';button.style.margin='3px 1px'; }, false);
		addEvent(button, 'mouseout', function(e) { button.style.border='1px outset';button.style.margin='4px 2px'; }, false);
		addEvent(button, 'mousedown', function(e) { button.style.border='2px inset';button.style.margin='3px 1px'; }, false);
		addEvent(button, 'mouseup', function(e) { button.style.border='2px outset';button.style.margin='3px 1px'; }, false);
		return button;
}
...
textArea.prototype.endDrag = function (event) {
	...
	//removeEvent(document, 'mousemove', null, false)
	//removeEvent(document, 'mouseup', null, false)
